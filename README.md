...
Execute the sta/scripts/sta.tcl script and observe the multiple reports. Notice how for each of the reports produced by STA in Lab 1 now we have 2 files, one per each of the process corners. Check out the equivalent file across both corners and notice how the propagation delays are different.
